...
cmd_audit() {
  ...
  local total
  total=$(wc -l <<< "$repos")
  log "scanning $total repos for: $OLD_EMAILS"
  ...
  while read -r slug; do
    i=$((i + 1))
    local created emails scanned hits
  ...
  local listed scanned_n
  listed=$(wc -l <<< "$repos")
  scanned_n=$(( $(wc -l < "$out") - 1 ))
  [ "$listed" -eq "$scanned_n" ] || warn "listed $listed repos but scanned $scanned_n — re-run audit"
